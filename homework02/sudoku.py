...
def read_sudoku(filename) -> List[List[str]]:
...
def find_empty_positions(grid: List[List[str]]) -> Union[None, Tuple[int, int]]:
...
def solve(grid: List[List[str]]) -> Union[None, List[List[str]]]:
